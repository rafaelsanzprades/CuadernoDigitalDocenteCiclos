        ...
        <main className="flex-1 p-8 content-area space-y-6">
          ...
          <div className="glass-card p-6">
            ...
            <div className="space-y-5">
              {Object.keys(ra_info).map(ra_id => {
                ...
                return (
                  <div key={ra_id} className="bg-white/5 rounded-lg border border-white/10 p-4">
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center gap-3">
                        <span className="font-bold text-white">{ra_id}</span>
                        <span className="text-xs text-gray-500">({info.pond.toFixed(1)}%)</span>
                        <span className="text-sm text-gray-400 truncate max-w-md">{info.desc}</span>
                      </div>
                      <div className="flex items-center gap-4 text-xs">
                        <span className="text-gray-500">Trimestres: {tris.join(', ')}</span>
                      </div>
                    </div>

                    {/* Bar visualization 0-10 */}
                    <div className="relative h-8 bg-black/40 rounded-full border border-white/10 overflow-hidden">
          ...
          <div className="space-y-4">
            {df_evaluable.map((al: any) => {
              ...
              return (
                <details key={al_id} open className="eval-details group bg-white/5 rounded-lg border border-white/10 overflow-hidden open:bg-white/10 transition-colors">
                  <summary className="p-4 cursor-pointer flex items-center justify-between font-semibold text-lg select-none hover:bg-white/5">
                    ...
                    <div className="flex items-center gap-6 text-sm">
                      <div className="flex flex-col items-end">
                        <span className="text-gray-400 text-xs tracking-wider mb-1">Porcentaje medio en RA's</span>
