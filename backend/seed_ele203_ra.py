...
def seed():
    ...
    print("Iniciando carga de Resultados de Aprendizaje para ELE203 GM...")
    ...
    # 2. Asegurar que el Ciclo ELE203 existe
    degree = db.query(Degree).filter_by(name="Instalaciones de Telecomunicaciones").first()
    if not degree:
        degree = Degree(family_id=family.id, level=NivelFP.MEDIO, name="Instalaciones de Telecomunicaciones", hours=2000)
        db.add(degree)
        db.commit()

    # 3. Leer Excel (Usaremos el archivo RA-CE-GM-GS.xlsx que contiene los datos de ELE203)
    xls_path = r'C:\GD-rsp\APP\RF Datos pdf xlsx\RA-CE-GM-GS.xlsx'
    try:
        df = pd.read_excel(xls_path, sheet_name='GM')
    except Exception as e:
        print(f"Error al leer el archivo Excel: {e}")
        db.close()
        return

    # Filtrar solo ELE203 (LFP)
    df_ele = df[df['Ciclo'].str.contains('ELE203', na=False)]
    ...
    for index, row in df_ele.iterrows():
        ...
        # Extraer el número del RA (ej. ELE203-0156-1 -> 1)
        ra_num = 0
        if '-' in cod_resultado:
            try:
        ...
        if not module:
            # Creamos el módulo temporalmente con un nombre por defecto; luego se puede editar o cargar el nombre real
    ...
    db.commit()
    print(f"✅ Se han verificado/creado {modulos_creados} módulos y {ras_creados} Resultados de Aprendizaje (RA) en la BBDD.")
    db.close()
