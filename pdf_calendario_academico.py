...
def _draw_page_decorations(canv, doc):
    ...
    canv.drawRightString(W - 1 * cm, 2 * cm, doc.cal_pie)
...
def generar_pdf_calendario(info_modulo, info_fechas, planning_ledger, calendar_notes):
    ...
    ROW_MES   = 1.5 * cm   # Fila del nombre del mes  (≈ 3× la normal)
    ROW_HEAD  = 1 * cm  # Fila de Lun/Mar/Mié…
    ROW_DIAS  = 0.75 * cm  # Fila de números de día
    ROW_UD    = 1.75 * cm   # Fila de UD (≈ 2× la normal)
